...
/** @class MultiIndexLimiter
    ...
    @details When constructing or adapting MultiIndex sets, some applications
       may need to impose additional requirements on what terms should be
       included.  This class provides a mechanism for adding that information to
       the MultiIndexSet.  The pure virtual function IsFeasible is overloaded by
       children to test individual multiindices.  If IsFeasible returns false
       for a given MultiIndex, the multiindex will not be included as an active
       member of the MultiIndexSet.
...
/** @class TotalOrderLimiter
    ...
    @details This limter only allows terms that satisfy \f$\|\mathbf{j}\|_1\leq
       p_U\f$, where \f$\mathbf{j}\f$ is the multiindex, and \f$p_U\f$ is a
       nonnegative integer passed to the constructor of this class.
 */
...
/** @class DimensionLimiter
   ...
   @details This limiter only allows terms that satisfy \f$\mathbf{j}_d = 0 \f$
      for \f$d<D_L\f$ or \f$d>=D_L+M\f$ for a lower bound \f$D_L\f$ and length
      \f$M\f$.
 */
...
/** @class MaxOrderLimiter
   ...
   @details This limter only allows terms that satisfy \f$\mathbf{j}_i\leq
      p_i\f$ for \f$i\in \{1,2,\ldots,D\}\f$, where \f$p\f$ is a vector of upper
      bounds.
 */
  ...
  public:
    ...
    MaxOrderLimiter(Eigen::VectorXi const & maxOrdersIn) : maxOrder(-1),
                                                           maxOrders(maxOrdersIn),
                                                           vectorMin(maxOrdersIn.minCoeff()) {};
...
/** @class NoLimiter
   ...
   @details This class is used as a default in many places where a limiter is
      not always needed.  IsFeasible will return true for any multiindex.
 */
...
/** @class AndLimiter
   ...
   @details This class will return true if both limiters given to the
      constructor return true.
 */
 class AndLimiter : public MultiIndexLimiter{

  public:
    AndLimiter(std::shared_ptr<MultiIndexLimiter> limitA,
               std::shared_ptr<MultiIndexLimiter> limitB) : a(limitA),
                                                            b(limitB){};
...
/** @class OrLimiter
   ...
   @details This class will return true if either of the limiters given to the
      constructor return true.
 */
  class OrLimiter : public MultiIndexLimiter{

  public:
    OrLimiter(std::shared_ptr<MultiIndexLimiter> limitA,
              std::shared_ptr<MultiIndexLimiter> limitB) : a(limitA),
                                                           b(limitB){};
  ...
  /** @class XorLimiter
   ...
   @details This class will return true if exactly one of the limiters given to
      the constructor returns true.
 */
  class XorLimiter : public MultiIndexLimiter{

  public:
    XorLimiter(std::shared_ptr<MultiIndexLimiter> limitA,
               std::shared_ptr<MultiIndexLimiter> limitB) : a(limitA),
                                                            b(limitB){};
