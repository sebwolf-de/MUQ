...
struct LinearOperatorFactory<Eigen::Matrix<ScalarType, Eigen::Dynamic, Eigen::Dynamic>> {
  ...
  {
      return std::make_shared<muq::Utilities::EigenLinearOperator<Eigen::Matrix<ScalarType, Eigen::Dynamic, Eigen::Dynamic>>>(A);
  }
};
...
struct LinearOperatorFactory<Eigen::SparseMatrix<ScalarType>> {
  ...
  {
    return std::make_shared<muq::Utilities::EigenLinearOperator<Eigen::SparseMatrix<ScalarType>>>(A);
  }
};

    
//template<typename Derived>
//std::shared_ptr<LinearOperator> muq::Utilities::LinearOperatorFactory<Eigen::MatrixBase<Derived>>::Create(Eigen::MatrixBase<Derived> const& A)
//{
//    return std::make_shared<muq::Utilities::EigenLinearOperator<Eigen::MatrixBase<Derived>>>(A);
//}
