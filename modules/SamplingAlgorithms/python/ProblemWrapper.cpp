...
void PythonBindings::ProblemWrapper(py::module &m) {
  ...
  sampProb
    ...
    expenProb
      ...
      .def(py::init( [] (std::shared_ptr<muq::Modeling::ModPiece> target, Eigen::VectorXd const& centroid, py::dict d, std::shared_ptr<parcer::Communicator> comm) { return new ExpensiveSamplingProblem(target, centroid, ConvertDictToPtree(d), comm); }))
