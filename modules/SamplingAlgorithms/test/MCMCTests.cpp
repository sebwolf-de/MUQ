...
TEST(MCMC, ParallelMHKernel) {
  ...
  pt::ptree pt;
  pt.put("MyMCMC.NumSamples", N); // number of Monte Carlo samples
  pt.put("MyMCMC.PrintLevel",3);
  pt.put("MyMCMC.KernelList", "Kernel1"); // the transition kernel
  pt.put("MyMCMC.Kernel1.Method","ParallelMHKernel");
  pt.put("MyMCMC.Kernel1.ProposalsPerStep",20);
  pt.put("MyMCMC.Kernel1.ParallelEvaluations", true);

  pt.put("MyMCMC.Kernel1.Proposal", "MyProposal"); // the proposal
  pt.put("MyMCMC.Kernel1.MyProposal.Method", "MHProposal");
  pt.put("MyMCMC.Kernel1.MyProposal.ProposalVariance", 2.0); // the variance of the isotropic MH proposal

  pt.put("MyMCMC.Kernel1.Auxillary Proposal", "AuxProposal");
  pt.put("MyMCMC.Kernel1.AuxProposal.Method", "MHProposal");
  pt.put("MyMCMC.Kernel1.AuxProposal.ProposalVariance", 1e-9); // the variance of the isotropic MH proposal
  ...
  // starting point
  const Eigen::VectorXd start = std::make_shared<Gaussian>(mu)->Sample();
  ...
  EXPECT_TRUE(pt.get<int>("MyMCMC.NumSamples")<=samps->size() );

  //std::cout << "Effective sample size = " << samps->ESS().transpose() << std::endl;
  //boost::any anyMean = samps.Mean();
  Eigen::VectorXd mean = samps->Mean();
  EXPECT_NEAR(mu(0), mean(0), 1e-1);
  EXPECT_NEAR(mu(1), mean(1), 1e-1);
