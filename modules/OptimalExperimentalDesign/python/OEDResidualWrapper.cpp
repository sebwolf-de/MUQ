...
void PythonBindings::OEDResidualWrapper(py::module &m) {
  ...
  res.def(py::init( [] (std::shared_ptr<Distribution> const& likelihood, std::shared_ptr<Distribution> const& evidence, std::shared_ptr<Distribution> const& biasing, py::dict d) { return new OEDResidual(likelihood, evidence, biasing, ConvertDictToPtree(d)); }));
  ...
  res.def(py::init( [] (std::shared_ptr<Distribution> const& likelihood, std::shared_ptr<Distribution> const& evidence, std::shared_ptr<Distribution> const& biasing, py::dict d, std::shared_ptr<parcer::Communicator> const& comm) { return new OEDResidual(likelihood, evidence, biasing, ConvertDictToPtree(d), comm); }));
#endif
}
