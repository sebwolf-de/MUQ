...
SumPiece::SumPiece(unsigned int dim, unsigned int numInputs) : ModPiece(dim*Eigen::VectorXi::Ones(dim),
...
{
  gradient = input.at(inputDimWrt).get().array() * sensitivity.array();
}
...
void SumPiece::JacobianImpl(unsigned int                const  outputDimWrt,
                               ...
                               ref_vector<Eigen::VectorXd> const& input)
{
  jacobian = Eigen::MatrixXd::Ones(inputSizes(inputDimWrt),inputSizes(inputDimWrt));
}
