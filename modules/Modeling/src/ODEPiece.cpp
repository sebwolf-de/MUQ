...
ODEPiece::ODEPiece(std::shared_ptr<ModPiece>   const& rhsIn,
...
double ODEPiece::GetStartTime(Eigen::VectorXd             const& evalTimesIn,
                              boost::property_tree::ptree const& opts)
{
  ...
  if(opts.count("StartTime")>0){
    ...
    if(startTime>evalTimesIn(0) + std::numeric_limits<double>::epsilon()){
      throw std::runtime_error("ERROR: Start time specified in options to ODEPiece is after first evaluation time.");
    }
...
Eigen::VectorXi ODEPiece::GetInputSizes(std::shared_ptr<ModPiece> const& rhsIn,
                                        bool                             isAuto)
{
  if(isAuto){
...
Eigen::VectorXi ODEPiece::GetOutputSizes(std::shared_ptr<ModPiece> const& rhsIn,
...
void ODEPiece::NonlinearSolverOptions::SetOptions(boost::property_tree::ptree const& opts)
{
  method = opts.get("Method", "Newton");
  maxIters = opts.get("MaximumIterations", -1);
}

void ODEPiece::LinearSolverOptions::SetOptions(boost::property_tree::ptree const& opts)
{
  method = opts.get("Method", "Dense");
  maxIters = opts.get("MaximumIterations", -1);
}

void ODEPiece::IntegratorOptions::SetOptions(boost::property_tree::ptree const& opts)
{
...
std::pair<SUNLinearSolver, SUNMatrix> ODEPiece::CreateLinearSolver(void *cvode_mem, N_Vector const& state)
{
  int flag;
...
SUNNonlinearSolver ODEPiece::CreateNonlinearSolver(void *cvode_mem, N_Vector const& state)
{
 int flag;
...
std::pair<SUNLinearSolver, SUNMatrix> ODEPiece::CreateAdjointLinearSolver(void *cvode_mem, N_Vector const& state, int indexB)
{
  int flag;
  int stateSize = NV_LENGTH_S(state);
...
SUNNonlinearSolver ODEPiece::CreateAdjointNonlinearSolver(void *cvode_mem, N_Vector const& state, int indexB)
{
 int flag;
...
void ODEPiece::EvaluateImpl(ref_vector<Eigen::VectorXd> const& inputs)
{
...
void ODEPiece::GradientImpl(unsigned int outWrt,
                            unsigned int inWrt,
...
void ODEPiece::JacobianImpl(unsigned int outWrt,
                            unsigned int inWrt,
                            ref_vector<Eigen::VectorXd> const& inputs)
{
  // Used for monitoring the success of CVODE calls
...
int ODEPiece::Interface::RHSJacobianAction(N_Vector v,  // The vector we want to apply the Jacobian to
...
int ODEPiece::Interface::RHSJacobian(double time,
                                     N_Vector state,
                                     N_Vector rhs,
                                     SUNMatrix jac,
                                     void *userData,
...
int ODEPiece::Interface::SensitivityRHS(int Ns, realtype time, N_Vector y, N_Vector ydot, N_Vector *ys, N_Vector *ySdot, void *user_data, N_Vector tmp1, N_Vector tmp2) {
...
int ODEPiece::Interface::AdjointRHS(realtype time, N_Vector state, N_Vector lambda, N_Vector deriv, void *user_data) {
...
int ODEPiece::Interface::AdjointJacobianAction(N_Vector target, N_Vector output, realtype time, N_Vector state, N_Vector lambda, N_Vector adjRhs, void *user_data, N_Vector tmp) {
...
int ODEPiece::Interface::AdjointJacobian(double time, N_Vector state, N_Vector lambda, N_Vector rhs, SUNMatrix jac, void *user_data, N_Vector tmp1, N_Vector tmp2, N_Vector tmp3) {
...
int ODEPiece::Interface::AdjointQuad(realtype time, N_Vector state, N_Vector lambda, N_Vector quadRhs, void *user_data) {
...
void ODEPiece::CheckFlag(void *returnvalue, const char *funcname, int opt)
{
  int *retval;
  std::stringstream msg;
